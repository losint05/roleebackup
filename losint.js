...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "losint ❤️ kimse" }, status: "idle" });
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Losint Guard Vurdu Gol Oldu" }).catch();
};
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    Database.find({guildID: ayarlar.guildID}).sort().exec((err, roles) => {
      roles.filter(r => !guild.roles.cache.has(r.roleID) && Date.now()-r.time > 1000*60*5).forEach(r => {//1 saatte bir alır. Süreyi değiştirebilirsiinz.
        Database.findOneAndDelete({roleID: r.roleID});
      });
    });
    let logKanali = client.channels.cache.get(ayarlar.logChannelID);
    if (logKanali) { logKanali.send(new MessageEmbed().setColor("RANDOM").setTitle("Rol Veri Tabanı Düzenlendi"))}
